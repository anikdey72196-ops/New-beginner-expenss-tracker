...
## 2024-05-14 - Missing Application-Layer Form Length Boundaries (DoS Risk)
**Vulnerability:** Registration, expense additions, and API endpoints did not limit string input lengths (e.g., username, password, description). When these unbound inputs were persisted via SQLAlchemy (`db.session.commit()`), they exceeded strictly typed database column lengths (e.g. `String(80)`, `String(255)`), generating a generic `DataError` / `OperationalError` which crashed the server resulting in HTTP 500 errors. Also, unbounded hashing algorithms (like `generate_password_hash`) expose the application to DoS attacks via CPU exhaustion from hashing incredibly long strings.
**Learning:** Even though `wtforms.validators.DataRequired` ensures presence of form data, it does not cap its length. SQLAlchemy strict type declarations in `models.py` only define the database side, leaving a gap where oversized inputs can cause server-side unhandled exception failures.
**Prevention:** All user-provided string inputs in both web forms (WTForms) and JSON API inputs must contain application-layer boundary checking (e.g., using `Length` validators or manual length checks in endpoints) to reject malformed/oversized payloads gracefully (HTTP 400 or flash warning) before they reach DB insertion operations or intensive CPU functions like hashing.
