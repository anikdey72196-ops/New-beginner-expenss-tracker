...
## 2024-07-21 - [Fix] Insecure Password Truncation and Type Checking Vulnerabilities
**Vulnerability:** The application was missing strict length validation limit for password sizes, which allows Denial of Service (DoS) attacks via CPU exhaustion when hashing overly long passwords. Also, bcrypt silently truncates passwords longer than 72 bytes which will cause validation issues later on. Furthermore, duplicated definitions in `form.py` were observed alongside missing type checking for API inputs.
**Learning:** Application-layer boundary checks (e.g. Length validators) must be strictly enforced on inputs like usernames and passwords to prevent unhandled database DataError crashes on oversized inputs and DoS attacks. JSON API inputs must also be safely type-checked.
**Prevention:** Always enforce explicit length constraints (max 72 characters for passwords) for passwords, particularly for strings that are passed to expensive cryptographic hashing functions (like `werkzeug.security.generate_password_hash`). Always check for string types before doing a length evaluation.
