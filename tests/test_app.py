...
def test_api_signup_oversized_payloads():
    ...
    with api_app.test_client() as api_client:
        ...
        assert response.get_json()["error"] == "Username must be a string between 3 and 80 characters."
        ...
        assert response.status_code == 400
        assert response.get_json()["error"] == "Password must be a string between 8 and 72 characters."
